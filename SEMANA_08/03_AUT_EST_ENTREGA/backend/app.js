...
app.get('/experiencia', (req, res) => {
	...
	var sql = 'SELECT * FROM experiencia ORDER BY nome_da_empresa COLLATE NOCASE';
		db.all(sql, [],  (err, rows ) => {
			if (err) {
				throw err;
			}
...
app.post('/atualizaUsuario', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); 
	sql = `UPDATE experiencia SET cargo = "${req.body.cargo}" WHERE nome_da_empresa = "${req.body.nome_da_empresa}" `;
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
